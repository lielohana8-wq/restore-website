...
/* ─── GitHub Config ─── */
const GH_OWNER = "lielohana8-wq";
const GH_REPO = "restore-website";
...
/* ─── Save to GitHub (triggers Vercel rebuild) ─── */
async function saveToGitHub(token, data) {
  ...
  const body = { message: "עדכון אתר מפאנל ניהול", content, ...(sha ? { sha } : {}) };
  const res = await fetch(url, { method: "PUT", headers, body: JSON.stringify(body) });
  return res.ok;
}

/* ─── Image compression ─── */
function compressImg(file, maxW = 500) {
...
const DEFAULTS = {
  ...
  services:[
    ...
    {name:"ניקוי מזרנים",desc:"חיטוי עמוק והסרת קרדית אבק, אלרגנים וכתמים.",price:150,accent:"#6DC489"},
...
const PROCESS = [
  ...
  {step:"02",title:"הזרקת חומר ניקוי",desc:"חומר מקצועי היפואלרגני לעומק הסיבים. מפרק שומנים, כתמים וחיידקים.",icon:"💧"},
  {step:"03",title:"שאיבה מקצועית",desc:"שואבים הכל בציוד תעשייתי — 95% מהלחות נשאבת.",icon:"⚡"},
  {step:"04",title:"חיטוי, ייבוש ובישום",desc:"חיטוי אנטיבקטריאלי, ייבוש מואץ ובישום. 4-6 שעות — כמו חדשה.",icon:"✨"},
];
...
export default function App(){
  ...
  useEffect(()=>{ loadData().then(d=>{ if(d&&d.phone) setData({...DEFAULTS,...d}); setLoading(false); }); },[]);
  ...
  return<div><style>{CSS}</style>
    ...
    {page==="admin"&&<Admin data={data} setData={setData} back={()=>{loadData().then(d=>{if(d&&d.phone)setData({...DEFAULTS,...d});});setPage("site");}}/>}
  </div>;
}
...
<section style={{padding:"80px 0",position:"relative",overflow:"hidden"}}><div style={{position:"absolute",inset:0,backgroundImage:"url(/img/lion-office.png)",backgroundSize:"cover",backgroundPosition:"center 25%",opacity:.6}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(to left,rgba(9,9,9,.1),rgba(9,9,9,.55) 55%)"}}/><div className="mx" style={{position:"relative",zIndex:2}}><div className="m-center" style={{maxWidth:480}}><Fade><h2 style={{fontSize:"clamp(22px,4vw,30px)",color:"#fff",marginBottom:10}}>רוצים הצעת מחיר?<br/><span style={{color:"#C8A44E"}}>האריה מחכה 🦁</span></h2></Fade><Fade d={.1}><p style={{color:"rgba(255,255,255,.4)",fontSize:14.5,marginBottom:24,lineHeight:1.75}}>שלחו תמונה — הצעה מדויקת תוך דקות.</p></Fade><Fade d={.2}><div className="m-stack" style={{display:"flex",gap:10}}><a href={wa} target="_blank" rel="noopener" className="btn btn-g m-full">💬 שלחו תמונה</a><a href={`tel:${D.phone.replace(/-/g,"")}`} className="btn btn-a m-full">📞 חייגו</a></div></Fade></div></div></section>
...
<section className="sec" id="faq"><div className="mx" style={{maxWidth:680}}><Fade><STit>שאלות נפוצות</STit></Fade>{[["כמה זמן לוקח?","30-60 דקות. ייבוש 4-6 שעות."],["החומרים בטוחים?","כן — היפואלרגניים."],["כל סוגי ספות?","כן — בד, עור, אימפלה, קטיפה."],["כמה עולה?","ספה מ-₪"+(D.services?.[0]?.price||250)+". שלחו תמונה."],["לאן מגיעים?","פריסה ארצית."]].map(([q,a],i)=><Fade key={i} d={i*.04}><FaqItem q={q} a={a}/></Fade>)}</div></section>
...
function Admin({data,setData,back}){
  ...
  const[token,setToken]=useState(localStorage.getItem("r_gh_token")||"");
  const show=m=>{setToast(m);setTimeout(()=>setToast(""),4000);};
  const del=id=>{const u=leads.filter(l=>l.id!==id);setLeads(u);sL(u);};
  ...
  const saveAll=async()=>{
    if(!token){show("⚠️ הכנס GitHub Token קודם בטאב הגדרות");setTab("setup");return;}
    setSaving(true); show("שומר ומפרסם...");
    try {
      const ok = await saveToGitHub(token, d);
      if(ok){ setData(d); show("✅ נשמר! האתר יתעדכן תוך דקה לכל המבקרים"); }
      else { show("❌ שגיאה — בדוק שה-Token תקין"); }
    } catch(e){ show("❌ "+e.message); }
    setSaving(false);
  };

  const handleImg=async(i,key,file)=>{
    if(!file)return; show("מכווץ תמונה...");
    const compressed = await compressImg(file, 500);
    upB(i, key, compressed); show("✅ תמונה נוספה — לחץ פרסם");
  };
...
return<div style={{minHeight:"100vh",background:"#090909",direction:"rtl"}}>
  {toast&&<div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:10000,background:toast.includes("❌")?"#DC2626":"#059669",color:"#fff",padding:"12px 24px",borderRadius:12,fontSize:14,fontFamily:"'Heebo'",fontWeight:600,boxShadow:"0 4px 20px rgba(0,0,0,.4)",maxWidth:"90vw",textAlign:"center"}}>{toast}</div>}
  <div style={{background:"rgba(14,26,43,.6)",borderBottom:"1px solid rgba(200,164,78,.06)",padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}><img src="/img/logo.png" style={{height:36,mixBlendMode:"screen"}} alt=""/><span style={{fontFamily:"'Heebo'",fontSize:14,fontWeight:800,color:"#C8A44E"}}>פאנל ניהול</span>{token&&<span style={{fontSize:10,color:"#25D366"}}>● מחובר</span>}</div>
    <div style={{display:"flex",gap:8}}><button onClick={saveAll} disabled={saving} className="btn btn-g" style={{padding:"8px 20px",fontSize:14,opacity:saving?.6:1}}>{saving?"שומר...":"🚀 פרסם לכולם"}</button><button onClick={back} style={{background:"none",border:"1px solid rgba(255,255,255,.06)",borderRadius:10,padding:"8px 14px",color:"rgba(255,255,255,.35)",cursor:"pointer",fontSize:12}}>← חזרה</button></div>
  </div>
  <div style={{display:"flex",gap:4,padding:"12px 20px",borderBottom:"1px solid rgba(255,255,255,.03)",flexWrap:"wrap"}}>{[["setup","🔑 הגדרות"],["contact","📞 קשר"],["services","💰 שירותים"],["reviews","⭐ ביקורות"],["ba","📸 לפני/אחרי"],["areas","🗺️ אזורים"],["leads","📋 לידים ("+leads.length+")"]].map(([id,l])=><button key={id} onClick={()=>setTab(id)} style={{padding:"10px 14px",borderRadius:10,border:"none",cursor:"pointer",fontSize:12,fontFamily:"'Heebo'",fontWeight:600,background:tab===id?"rgba(200,164,78,.1)":"transparent",color:tab===id?"#C8A44E":"rgba(255,255,255,.3)"}}>{l}</button>)}</div>
  <div style={{maxWidth:820,margin:"0 auto",padding:20}}>

    {tab==="setup"&&<div><h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>🔑 חיבור GitHub (פעם אחת)</h3><div style={box}>
      {token?<><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}><div style={{width:10,height:10,borderRadius:"50%",background:"#25D366"}}/><span style={{color:"#25D366",fontFamily:"'Heebo'",fontWeight:700}}>מחובר ✅</span></div><p style={{fontSize:13,color:"rgba(255,255,255,.35)",lineHeight:1.8}}>כל שינוי שתפרסם ישמר ויהיה גלוי <strong style={{color:"#fff"}}>לכל המבקרים</strong> באתר תוך ~דקה.</p><button onClick={()=>{localStorage.removeItem("r_gh_token");setToken("");}} style={{marginTop:12,padding:"6px 14px",borderRadius:8,background:"rgba(248,113,113,.06)",border:"none",color:"#F87171",fontSize:12,cursor:"pointer"}}>נתק</button></>
      :<><p style={{fontSize:14,color:"rgba(255,255,255,.4)",lineHeight:2,marginBottom:16}}>צריך ליצור GitHub Token כדי שהשינויים ישמרו <strong style={{color:"#fff"}}>לכל המבקרים</strong>.</p>
        <div style={{background:"rgba(0,0,0,.3)",borderRadius:12,padding:16,marginBottom:16}}>
          <p style={{fontSize:13,color:"#C8A44E",fontFamily:"'Heebo'",fontWeight:700,marginBottom:8}}>📋 הוראות (פעם אחת, 2 דקות):</p>
          <ol style={{fontSize:13,color:"rgba(255,255,255,.4)",lineHeight:2.2,paddingRight:20}}>
            <li>לכו ל: <a href="https://github.com/settings/tokens?type=beta" target="_blank" rel="noopener" style={{color:"#C8A44E",textDecoration:"underline"}}>github.com/settings/tokens</a></li>
            <li>לחצו <strong style={{color:"#fff"}}>Generate new token</strong></li>
            <li>שם: <code style={{background:"rgba(200,164,78,.1)",padding:"2px 6px",borderRadius:4,color:"#C8A44E"}}>restore-admin</code></li>
            <li>Repository access → <strong style={{color:"#fff"}}>Only select repositories</strong> → בחרו <strong style={{color:"#fff"}}>restore-website</strong></li>
            <li>Permissions → Contents → <strong style={{color:"#fff"}}>Read and write</strong></li>
            <li>לחצו <strong style={{color:"#fff"}}>Generate token</strong></li>
            <li>העתיקו את ה-Token והדביקו למטה</li>
          </ol>
        </div>
        <label style={lbl}>GitHub Token</label>
        <input style={{...inp,fontFamily:"monospace",fontSize:12,direction:"ltr",textAlign:"left"}} value={token} onChange={e=>setToken(e.target.value)} placeholder="github_pat_..."/>
        <button onClick={()=>{if(token.length>10){localStorage.setItem("r_gh_token",token);show("✅ Token נשמר!");}else{show("⚠️ Token לא תקין");}}} className="btn btn-a" style={{marginTop:8,padding:"10px 24px",fontSize:14}}>💾 שמור Token</button>
      </>}
    </div></div>}

    {tab==="contact"&&<div><h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>📞 פרטי קשר</h3><div style={box}><label style={lbl}>טלפון</label><input style={inp} value={d.phone} onChange={e=>sd({...d,phone:e.target.value})}/><label style={lbl}>וואטסאפ (ללא +)</label><input style={inp} value={d.waNum} onChange={e=>sd({...d,waNum:e.target.value})}/></div></div>}

    {tab==="services"&&<div><h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>💰 שירותים <button onClick={()=>sd({...d,services:[...(d.services||[]),{name:"חדש",desc:"תיאור",price:100,accent:"#C8A44E"}]})} style={{fontSize:12,background:"rgba(200,164,78,.1)",border:"none",color:"#C8A44E",padding:"4px 12px",borderRadius:8,cursor:"pointer"}}>+ הוסף</button></h3>{(d.services||[]).map((s,i)=><div key={i} style={box}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><strong style={{color:"#C8A44E"}}>שירות {i+1}</strong><button onClick={()=>sd({...d,services:d.services.filter((_,j)=>j!==i)})} style={{background:"none",border:"none",color:"#F87171",cursor:"pointer",fontSize:12}}>🗑</button></div><label style={lbl}>שם</label><input style={inp} value={s.name} onChange={e=>upS(i,"name",e.target.value)}/><label style={lbl}>תיאור</label><textarea style={{...inp,height:60,resize:"vertical"}} value={s.desc} onChange={e=>upS(i,"desc",e.target.value)}/><div style={{display:"flex",gap:8}}><div style={{flex:1}}><label style={lbl}>מחיר ₪</label><input style={inp} type="number" value={s.price} onChange={e=>upS(i,"price",e.target.value)}/></div><div style={{flex:1}}><label style={lbl}>צבע</label><input style={{...inp,padding:4,height:38}} type="color" value={s.accent||"#C8A44E"} onChange={e=>upS(i,"accent",e.target.value)}/></div></div></div>)}</div>}

    {tab==="reviews"&&<div><h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>⭐ ביקורות <button onClick={()=>sd({...d,reviews:[...(d.reviews||[]),{n:"שם",t:"ביקורת",s:"שירות"}]})} style={{fontSize:12,background:"rgba(200,164,78,.1)",border:"none",color:"#C8A44E",padding:"4px 12px",borderRadius:8,cursor:"pointer"}}>+ הוסף</button></h3>{(d.reviews||[]).map((r,i)=><div key={i} style={box}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><strong style={{color:"#C8A44E"}}>ביקורת {i+1}</strong><button onClick={()=>sd({...d,reviews:d.reviews.filter((_,j)=>j!==i)})} style={{background:"none",border:"none",color:"#F87171",cursor:"pointer",fontSize:12}}>🗑</button></div><label style={lbl}>שם</label><input style={inp} value={r.n} onChange={e=>upR(i,"n",e.target.value)}/><label style={lbl}>ביקורת</label><textarea style={{...inp,height:60,resize:"vertical"}} value={r.t} onChange={e=>upR(i,"t",e.target.value)}/><label style={lbl}>שירות</label><input style={inp} value={r.s} onChange={e=>upR(i,"s",e.target.value)}/></div>)}</div>}

    {tab==="ba"&&<div><h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>📸 לפני / אחרי <button onClick={()=>sd({...d,beforeAfter:[...(d.beforeAfter||[]),{title:"חדש",desc:"תיאור",bc:"#555",ac:"#aaa",imgBefore:"",imgAfter:""}]})} style={{fontSize:12,background:"rgba(200,164,78,.1)",border:"none",color:"#C8A44E",padding:"4px 12px",borderRadius:8,cursor:"pointer"}}>+ הוסף</button></h3>{(d.beforeAfter||[]).map((b,i)=><div key={i} style={box}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><strong style={{color:"#C8A44E"}}>פריט {i+1}</strong><button onClick={()=>sd({...d,beforeAfter:d.beforeAfter.filter((_,j)=>j!==i)})} style={{background:"none",border:"none",color:"#F87171",cursor:"pointer",fontSize:12}}>🗑</button></div><label style={lbl}>כותרת</label><input style={inp} value={b.title} onChange={e=>upB(i,"title",e.target.value)}/><label style={lbl}>תיאור</label><input style={inp} value={b.desc} onChange={e=>upB(i,"desc",e.target.value)}/>
    ...
    {tab==="areas"&&<div><h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>🗺️ אזורי שירות</h3><div style={box}><label style={lbl}>ערים (מופרדות ב-·)</label><textarea style={{...inp,height:100,resize:"vertical"}} value={d.areas||""} onChange={e=>sd({...d,areas:e.target.value})}/></div></div>}

    {tab==="leads"&&<div><h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>📋 לידים</h3>{leads.length===0?<div style={box}><p style={{textAlign:"center",color:"rgba(255,255,255,.2)",padding:16}}>אין לידים</p></div>:[...leads].reverse().map(l=><div key={l.id} style={box}><div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}><div><strong style={{color:"#fff"}}>{l.name}</strong><div style={{fontSize:13,color:"rgba(255,255,255,.3)"}}>{l.phone}</div>{l.service&&<div style={{fontSize:12,color:"#C8A44E"}}>{l.service}</div>}</div><div style={{display:"flex",gap:4,alignItems:"center"}}><select value={l.status} onChange={e=>upSt(l.id,e.target.value)} style={{padding:"4px 8px",borderRadius:6,border:"1px solid rgba(200,164,78,.06)",fontSize:11,background:"rgba(0,0,0,.3)",color:"#fff"}}><option value="new">🆕 חדש</option><option value="called">📞 נוצר קשר</option><option value="done">✅ הושלם</option></select><a href={`https://wa.me/972${l.phone.replace(/^0/,"").replace(/-/g,"")}`} target="_blank" rel="noopener" style={{padding:"4px 8px",borderRadius:6,background:"#25D366",color:"#fff",fontSize:10}}>💬</a><button onClick={()=>del(l.id)} style={{padding:"4px 8px",borderRadius:6,background:"rgba(248,113,113,.06)",border:"none",color:"#F87171",fontSize:10,cursor:"pointer"}}>🗑</button></div></div></div>)}</div>}
