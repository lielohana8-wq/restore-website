...
const DEF = {
  ...
  beforeAfter: [
    { title:"ניקוי ספת בד", desc:"3 שנים של כתמים → נעלמו", bc:"#6b5544", ac:"#c9b99a" },
    { title:"ניקוי ריפודי רכב", desc:"רכב מוזנח → כמו מהסלון", bc:"#484848", ac:"#aaa" },
    { title:"ניקוי שטיח", desc:"שטיח לזריקה → ניצל", bc:"#5c4e3f", ac:"#b8a990" },
    { title:"ניקוי מזרן", desc:"כתמים + ריח → נעלמו", bc:"#7a6e5e", ac:"#ddd0c0" },
  ],
...
/* ─── Storage ─── */
const getD = () => { try { return { ...DEF, ...JSON.parse(localStorage.getItem("r_cfg") || "{}") }; } catch { return DEF; } };
const setD = (d) => localStorage.setItem("r_cfg", JSON.stringify(d));
...
const PROCESS = [
  { step:"01", title:"אבחון ראשוני", desc:"צלמו את הספה ושלחו בוואטסאפ. נאבחן את סוג הבד, רמת הלכלוך ודרך הטיפול — בחינם.", icon:"🔍" },
  { step:"02", title:"הזרקת חומר ניקוי", desc:"מזריקים חומר ניקוי מקצועי היפואלרגני לעומק הסיבים. מפרק שומנים, כתמים וחיידקים.", icon:"💧" },
  { step:"03", title:"שאיבה מקצועית", desc:"שואבים את הלכלוך, החומר והמים בציוד תעשייתי — 95% מהלחות נשאבת.", icon:"⚡" },
  { step:"04", title:"חיטוי, ייבוש ובישום", desc:"חיטוי אנטיבקטריאלי, ייבוש מואץ ובישום קל. תוך 4-6 שעות — כמו חדשה.", icon:"✨" },
];
...
body{
  ...
  background:#090909;
  background-image:radial-gradient(ellipse at 15% 25%,rgba(200,164,78,.07) 0%,transparent 50%),radial-gradient(ellipse at 85% 65%,rgba(200,164,78,.05) 0%,transparent 45%),radial-gradient(ellipse at 70% 40%,rgba(14,26,43,.2) 0%,transparent 50%);
...
export default function App(){
  ...
  const[data,setData]=useState(getD());
  const save=d=>{setData(d);setD(d);};
  return<div><style>{CSS}</style>
    {page==="site"&&<Site data={data} goAdmin={()=>setPage("login")}/>}
    {page==="login"&&<Login pass={data.pass||DEF.pass} ok={()=>setPage("admin")} back={()=>setPage("site")}/>}
    {page==="admin"&&<Admin data={data} save={save} back={()=>{setData(getD());setPage("site");}}/>}
  </div>;
}

/* ═══ SITE ═══ */
function Site({data:D,goAdmin}){
  const wa=`https://wa.me/${D.waNum}`;
...
<div className="mx" style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:90}}>
  <img src="/img/logo.png" alt="RESTORE" onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{height:200,marginTop:50,cursor:"pointer",objectFit:"contain",animation:"glowPulse 4s ease-in-out infinite",filter:"drop-shadow(0 6px 20px rgba(0,0,0,.6))",position:"relative",zIndex:10,mixBlendMode:"screen"}}/>
...
{mm&&<><div onClick={()=>setMm(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:200}}/><div style={{position:"fixed",top:0,right:0,bottom:0,width:"min(280px,80vw)",background:"#0c0c0c",zIndex:201,padding:20,display:"flex",flexDirection:"column",animation:"slideR .25s ease",borderLeft:"1px solid rgba(200,164,78,.08)"}}>
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}><img src="/img/logo.png" style={{height:48,mixBlendMode:"screen"}} alt=""/><button onClick={()=>setMm(false)} style={{background:"none",border:"none",color:"#C8A44E",fontSize:22,cursor:"pointer"}}>✕</button></div>
  ...
  <a href={wa} target="_blank" rel="noopener" className="btn btn-g" style={{marginTop:16,justifyContent:"center"}}>💬 וואטסאפ</a>
</div></>}
...
<section style={{minHeight:"100vh",display:"flex",alignItems:"center",position:"relative",overflow:"hidden",padding:"100px 0 50px"}}>
  ...
  <div className="mx" style={{position:"relative",zIndex:2,width:"100%"}}><div className="m-center" style={{maxWidth:540}}>
...
<div style={{background:"rgba(14,26,43,.5)",padding:"28px 0"}}><div className="mx"><div className="m-col2" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,textAlign:"center"}}>
  {[["3000","+","לקוחות מרוצים"],["100","%","שביעות רצון"],[String(D.services?.[0]?.price||250),"₪","ניקוי ספה מ-"],["7","","ימים בשבוע"]].map(([n,s,l],i)=><div key={i}><div style={{fontFamily:"'Heebo'",fontSize:28,fontWeight:900,color:"#C8A44E"}}><Num to={+n} sfx={s}/></div><div style={{fontSize:12,color:"rgba(255,255,255,.25)",marginTop:4}}>{l}</div></div>)}
</div></div></div>
<div className="gold-line"/>
...
<section className="sec" style={{position:"relative",overflow:"hidden",backgroundImage:"linear-gradient(rgba(9,9,9,.5),rgba(9,9,9,.5)),url(/img/lion-ac.png)",backgroundSize:"cover",backgroundPosition:"center 30%"}}>
  <div className="mx" style={{position:"relative",zIndex:1}}>
    <Fade><STit sub="הספה מסריחה. המזרן מלא כתמים. המזגן פולט אבק. מוכר?">הבעיה שכולם מכירים</STit></Fade>
    <div className="m-col" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:48}}>
      {[["🤢","ריחות רעים","עובש, זיעה ואוכל ישן שנספגו עמוק בסיבים"],["🦠","חיידקים ואלרגנים","קרדית אבק וזיהומים שמסכנים את בריאות המשפחה"],["💧","כתמים עיקשים","קפה, שתן, שומן — שום חומר ביתי לא יעזור"]].map(([ic,t,d],i)=><Fade key={i} d={i*.07}><div className="crd" style={{padding:"28px 22px",textAlign:"center"}}><div style={{fontSize:36,marginBottom:12}}>{ic}</div><h3 style={{fontSize:17,fontWeight:700,marginBottom:8,color:"#fff"}}>{t}</h3><p style={{fontSize:14,color:"rgba(255,255,255,.3)",lineHeight:1.7}}>{d}</p></div></Fade>)}
    </div>
    <Fade d={.25}><div style={{textAlign:"center"}}><h3 style={{fontSize:"clamp(22px,4vw,32px)",color:"#fff",marginBottom:12}}>הפתרון? <span style={{color:"#C8A44E"}}>RESTORE</span></h3><p style={{color:"rgba(255,255,255,.3)",maxWidth:420,margin:"0 auto 24px"}}>אנחנו לא סתם מנקים — אנחנו משחזרים.</p><a href={wa} target="_blank" rel="noopener" className="btn btn-a">💬 בואו נשחזר</a></div></Fade>
  </div>
</section>

{/* SERVICES */}
<section className="sec" id="services" style={{position:"relative",overflow:"hidden"}}><div style={{position:"absolute",inset:0,backgroundImage:"url(/img/lion-van.png)",backgroundSize:"cover",backgroundPosition:"center 30%",opacity:.6}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(rgba(9,9,9,.35),rgba(9,9,9,.3))"}}/>
  <div className="mx" style={{position:"relative",zIndex:1}}>
    <Fade><STit sub="פתרון מקצועי לכל סוג — ציוד תעשייתי וחומרים בטוחים">השירותים שלנו</STit></Fade>
    <div className="m-col2" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
      {(D.services||DEF.services).map((s,i)=><Fade key={i} d={i*.05}><div className="crd" style={{padding:"26px 22px",cursor:"pointer",height:"100%",display:"flex",flexDirection:"column",borderRight:`3px solid ${s.accent||"#C8A44E"}`}} onClick={()=>window.open(wm("היי, מעוניין/ת ב"+s.name),"_blank")}><h3 style={{fontSize:17,fontWeight:700,color:"#fff",marginBottom:8}}>{s.name}</h3><p style={{fontSize:13.5,color:"rgba(255,255,255,.3)",lineHeight:1.7,marginBottom:"auto",paddingBottom:14}}>{s.desc}</p><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:12,borderTop:"1px solid rgba(200,164,78,.06)"}}><span style={{fontFamily:"'Heebo'",fontSize:22,fontWeight:900,color:s.accent||"#C8A44E"}}>₪{s.price}</span><span style={{fontSize:12,color:"rgba(255,255,255,.2)"}}>הזמינו ←</span></div></div></Fade>)}
    </div>
  </div>
</section>

{/* PROCESS */}
<section className="sec" id="process" style={{position:"relative",overflow:"hidden"}}><div style={{position:"absolute",inset:0,backgroundImage:"url(/img/lion-inspect.png)",backgroundSize:"cover",backgroundPosition:"center 20%",opacity:.55}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(rgba(9,9,9,.35),rgba(9,9,9,.3))"}}/>
  <div className="mx" style={{position:"relative",zIndex:1}}>
    <Fade><STit sub="4 שלבים מהשיחה הראשונה ועד לספה כמו חדשה">מה הספה שלכם עוברת?</STit></Fade>
    <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0,maxWidth:680,margin:"0 auto"}}>{PROCESS.map((p,i)=><Fade key={i} d={i*.1}><div style={{display:"flex",gap:20,padding:"28px 0",borderBottom:i<3?"1px solid rgba(200,164,78,.06)":"none"}}><div style={{flex:"0 0 64px",height:64,borderRadius:16,background:"rgba(9,9,9,.7)",backdropFilter:"blur(8px)",border:"1px solid rgba(200,164,78,.1)",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:24}}>{p.icon}</span></div><div style={{flex:1}}><span style={{fontFamily:"'Heebo'",fontSize:12,fontWeight:800,color:"#C8A44E",letterSpacing:1}}>שלב {p.step}</span><h3 style={{fontSize:18,fontWeight:800,color:"#fff",marginBottom:6}}>{p.title}</h3><p style={{fontSize:14,color:"rgba(255,255,255,.35)",lineHeight:1.75}}>{p.desc}</p></div></div></Fade>)}</div>
    <Fade d={.4}><div style={{textAlign:"center",marginTop:36}}><a href={wa} target="_blank" rel="noopener" className="btn btn-g" style={{fontSize:16,padding:"16px 36px"}}>💬 שלחו תמונה — קבלו הצעה תוך דקות</a></div></Fade>
  </div>
</section>

{/* CTA */}
<section style={{padding:"80px 0",position:"relative",overflow:"hidden"}}><div style={{position:"absolute",inset:0,backgroundImage:"url(/img/lion-office.png)",backgroundSize:"cover",backgroundPosition:"center 25%",opacity:.6}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(to left,rgba(9,9,9,.1),rgba(9,9,9,.55) 55%)"}}/>
  <div className="mx" style={{position:"relative",zIndex:2}}><div className="m-center" style={{maxWidth:480}}>
    ...
    <Fade d={.1}><p style={{color:"rgba(255,255,255,.4)",fontSize:14.5,marginBottom:24,lineHeight:1.75}}>שלחו תמונה — תקבלו הצעה מדויקת תוך דקות.</p></Fade>
    <Fade d={.2}><div className="m-stack" style={{display:"flex",gap:10}}><a href={wa} target="_blank" rel="noopener" className="btn btn-g m-full">💬 שלחו תמונה</a><a href={`tel:${D.phone.replace(/-/g,"")}`} className="btn btn-a m-full">📞 חייגו</a></div></Fade>
  </div></div>
</section>
...
{/* REVIEWS */}
<section className="sec" id="reviews" style={{background:"rgba(14,26,43,.25)"}}><div className="mx">
  <Fade><STit>מה הלקוחות אומרים</STit></Fade>
  <div className="m-col" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
    {(D.reviews||DEF.reviews).map((r,i)=><Fade key={i} d={i*.05}><div className="crd" style={{padding:"24px 20px",height:"100%",display:"flex",flexDirection:"column"}}><div style={{color:"#C8A44E",fontSize:12,letterSpacing:2,marginBottom:10}}>★★★★★</div><p style={{fontSize:14,lineHeight:1.85,color:"rgba(255,255,255,.5)",marginBottom:"auto",paddingBottom:14}}>״{r.t}״</p><div style={{display:"flex",alignItems:"center",gap:10,paddingTop:12,borderTop:"1px solid rgba(200,164,78,.06)"}}><div style={{width:34,height:34,borderRadius:10,background:"linear-gradient(135deg,#C8A44E,#9A7B30)",display:"flex",alignItems:"center",justifyContent:"center",color:"#0E1A2B",fontFamily:"'Heebo'",fontWeight:800,fontSize:14}}>{r.n[0]}</div><div><div style={{fontSize:13,fontFamily:"'Heebo'",fontWeight:700,color:"#fff"}}>{r.n}</div><div style={{fontSize:11,color:"rgba(255,255,255,.2)"}}>{r.s}</div></div></div></div></Fade>)}
  </div>
</div></section>

{/* FAQ */}
<section className="sec" id="faq"><div className="mx" style={{maxWidth:680}}>
  <Fade><STit>שאלות נפוצות</STit></Fade>
  {[["כמה זמן לוקח ניקוי ספה?","30-60 דקות. ייבוש: 4-6 שעות."],["החומרים בטוחים?","כן — היפואלרגניים, בטוחים לילדים ולחיות."],["מנקים כל סוגי ספות?","כן — בד, עור, אימפלה, קטיפה, דמוי עור."],["מה אם הכתם לא יורד?","מתחייבים למקסימום. מעדכנים מראש."],["כמה עולה?","ספה מ-₪"+((D.services||DEF.services)[0]?.price||250)+". שלחו תמונה בוואטסאפ להצעה."],["לאן מגיעים?","פריסה ארצית — כל הארץ."]].map(([q,a],i)=><Fade key={i} d={i*.04}><FaqItem q={q} a={a}/></Fade>)}
</div></section>

{/* AREAS */}
<section className="sec" id="areas" style={{background:"rgba(14,26,43,.25)"}}><div className="mx">
  <Fade><STit>🚐 פריסה ארצית</STit></Fade>
  <Fade d={.08}><div className="crd" style={{padding:"32px 36px",textAlign:"center"}}><p style={{fontSize:15,lineHeight:2.3,color:"rgba(255,255,255,.35)",fontFamily:"'Heebo'",fontWeight:500}}>{D.areas||DEF.areas}</p><a href={wa} target="_blank" rel="noopener" className="btn btn-g" style={{marginTop:20}}>💬 בדקו אם מגיעים אליכם</a></div></Fade>
</div></section>

{/* FINAL CTA */}
<section className="sec" style={{textAlign:"center",position:"relative",overflow:"hidden"}}><div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:400,height:400,borderRadius:"50%",background:"radial-gradient(circle,rgba(200,164,78,.04),transparent)"}}/>
  <div className="mx" style={{position:"relative",zIndex:1}}><Fade><img src="/img/logo.png" alt="" style={{height:150,objectFit:"contain",margin:"0 auto 24px",animation:"glowPulse 3s ease-in-out infinite",mixBlendMode:"screen"}}/></Fade>
  ...
  <Fade d={.3}><div className="m-stack" style={{display:"flex",gap:10,justifyContent:"center"}}><a href={wa} target="_blank" rel="noopener" className="btn btn-g m-full" style={{fontSize:17,padding:"18px 36px"}}>💬 הזמינו עכשיו</a><a href={`tel:${D.phone.replace(/-/g,"")}`} className="btn btn-o m-full" style={{fontSize:17,padding:"18px 36px"}}>📞 {D.phone}</a></div></Fade>
</div></section>

{/* FORM */}
<section className="sec" id="contact" style={{background:"rgba(14,26,43,.25)"}}><div className="mx">
  <Fade><STit>השאירו פרטים</STit></Fade>
  <Fade d={.1}><LeadForm services={D.services||DEF.services} wa={wa}/></Fade>
</div></section>
...
<footer style={{background:"rgba(0,0,0,.4)",padding:"48px 0 16px",borderTop:"1px solid rgba(200,164,78,.05)"}}><div className="mx">
  <div className="m-col" style={{display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr 1fr",gap:28,marginBottom:32}}>
    <div><img src="/img/logo.png" alt="" style={{height:52,marginBottom:14,mixBlendMode:"screen"}}/><p style={{fontSize:13,color:"rgba(255,255,255,.15)",lineHeight:1.8}}>שחזור מקצועי — אחריות מלאה.</p></div>
    <div><h4 style={{color:"#C8A44E",fontSize:12,marginBottom:12}}>שירותים</h4>{(D.services||DEF.services).map((s,i)=><div key={i} style={{fontSize:12,color:"rgba(255,255,255,.15)",marginBottom:5,cursor:"pointer"}} onClick={()=>go("services")}>{s.name}</div>)}</div>
    ...
    <div><h4 style={{color:"#C8A44E",fontSize:12,marginBottom:12}}>צור קשר</h4><a href={`tel:${D.phone.replace(/-/g,"")}`} style={{display:"block",fontSize:12,color:"rgba(255,255,255,.15)",marginBottom:5}}>📞 {D.phone}</a><a href={wa} target="_blank" rel="noopener" style={{display:"block",fontSize:12,color:"rgba(255,255,255,.15)",marginBottom:10}}>💬 וואטסאפ</a></div>
  </div>
  <div style={{borderTop:"1px solid rgba(255,255,255,.03)",paddingTop:12,display:"flex",justifyContent:"space-between",fontSize:10,color:"rgba(255,255,255,.08)"}}><span>© 2026 RESTORE</span><span onClick={goAdmin} style={{cursor:"pointer"}}>ניהול</span></div>
</div></footer>
...
function LeadForm({services,wa}){const[f,setF]=useState({name:"",phone:"",service:""});const[sent,setSent]=useState(false);const inp={width:"100%",padding:"14px 16px",borderRadius:12,border:"1px solid rgba(200,164,78,.08)",fontSize:14,fontFamily:"'Assistant'",background:"rgba(255,255,255,.02)",color:"#fff",direction:"rtl",outline:"none"};
...
return<form onSubmit={e=>{e.preventDefault();if(f.name&&f.phone)submit();}} style={{maxWidth:480,margin:"0 auto",background:"rgba(9,9,9,.5)",backdropFilter:"blur(12px)",borderRadius:20,padding:"36px 28px",border:"1px solid rgba(200,164,78,.08)",textAlign:"right"}}>
...
function Admin({data,save,back}){
  ...
  const saveAll=()=>{save(d);show("נשמר בהצלחה ✅");};
  const upS=(i,k,v)=>{const s=[...d.services];s[i]={...s[i],[k]:k==="price"?+v:v};setD2({...d,services:s});};
  const upR=(i,k,v)=>{const r=[...d.reviews];r[i]={...r[i],[k]:v};setD2({...d,reviews:r});};
  const upB=(i,k,v)=>{const b=[...d.beforeAfter];b[i]={...b[i],[k]:v};setD2({...d,beforeAfter:b});};
  const addS=()=>setD2({...d,services:[...d.services,{name:"שירות חדש",desc:"תיאור",price:100,accent:"#C8A44E"}]});
...
return<div style={{minHeight:"100vh",background:"#090909",direction:"rtl"}}>
  ...
  <div style={{background:"rgba(14,26,43,.6)",borderBottom:"1px solid rgba(200,164,78,.06)",padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
    <div style={{display:"flex",alignItems:"center",gap:10}}><img src="/img/logo.png" style={{height:36,mixBlendMode:"screen"}} alt=""/><span style={{fontFamily:"'Heebo'",fontSize:14,fontWeight:800,color:"#C8A44E"}}>פאנל ניהול</span></div>
    <div style={{display:"flex",gap:8}}><button onClick={saveAll} className="btn btn-a" style={{padding:"8px 16px",fontSize:13}}>💾 שמור שינויים</button><button onClick={back} style={{background:"none",border:"1px solid rgba(255,255,255,.06)",borderRadius:10,padding:"8px 14px",color:"rgba(255,255,255,.35)",cursor:"pointer",fontSize:12,fontFamily:"'Heebo'"}}>← חזרה</button></div>
  </div>
  <div style={{display:"flex",gap:4,padding:"12px 20px",borderBottom:"1px solid rgba(255,255,255,.03)",flexWrap:"wrap"}}>
    {[["leads","📋 לידים ("+leads.length+")"],["contact","📞 פרטי קשר"],["services","💰 שירותים"],["reviews","⭐ ביקורות"],["ba","📸 לפני/אחרי"],["areas","🗺️ אזורים"]].map(([id,l])=><button key={id} onClick={()=>setTab(id)} style={{padding:"10px 16px",borderRadius:10,border:"none",cursor:"pointer",fontSize:13,fontFamily:"'Heebo'",fontWeight:600,background:tab===id?"rgba(200,164,78,.1)":"transparent",color:tab===id?"#C8A44E":"rgba(255,255,255,.3)"}}>{l}</button>)}
  </div>
  <div style={{maxWidth:820,margin:"0 auto",padding:20}}>

    {tab==="leads"&&<div>
      <h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>📋 לידים</h3>
      {leads.length===0?<div style={box}><p style={{textAlign:"center",color:"rgba(255,255,255,.2)",padding:16}}>אין לידים עדיין</p></div>
      :[...leads].reverse().map(l=><div key={l.id} style={box}><div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
        <div><strong style={{color:"#fff"}}>{l.name}</strong><div style={{fontSize:13,color:"rgba(255,255,255,.3)"}}>{l.phone}</div>{l.service&&<div style={{fontSize:12,color:"#C8A44E"}}>{l.service}</div>}<div style={{fontSize:11,color:"rgba(255,255,255,.1)"}}>{new Date(l.date).toLocaleString("he-IL")}</div></div>
        <div style={{display:"flex",gap:4,alignItems:"center"}}><select value={l.status} onChange={e=>upSt(l.id,e.target.value)} style={{padding:"4px 8px",borderRadius:6,border:"1px solid rgba(200,164,78,.06)",fontSize:11,background:"rgba(0,0,0,.3)",color:"#fff"}}><option value="new">🆕 חדש</option><option value="called">📞 נוצר קשר</option><option value="done">✅ הושלם</option></select>
        <a href={`https://wa.me/972${l.phone.replace(/^0/,"").replace(/-/g,"")}`} target="_blank" rel="noopener" style={{padding:"4px 8px",borderRadius:6,background:"#25D366",color:"#fff",fontSize:10,fontWeight:700}}>💬</a>
        <button onClick={()=>del(l.id)} style={{padding:"4px 8px",borderRadius:6,background:"rgba(248,113,113,.06)",border:"none",color:"#F87171",fontSize:10,cursor:"pointer"}}>🗑</button></div>
      </div></div>)}
      {leads.length>0&&<button onClick={()=>{if(confirm("למחוק הכל?")){sL([]);setLeads([]);show("נמחק");}}} style={{padding:"8px 14px",borderRadius:8,background:"rgba(248,113,113,.06)",border:"none",color:"#F87171",fontSize:12,cursor:"pointer"}}>🗑 מחק הכל</button>}
    </div>}

    {tab==="contact"&&<div>
      <h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>📞 פרטי קשר</h3>
      <div style={box}>
        <label style={lbl}>מספר טלפון</label><input style={inp} value={d.phone} onChange={e=>setD2({...d,phone:e.target.value})}/>
        <label style={lbl}>מספר וואטסאפ (ללא +)</label><input style={inp} value={d.waNum} onChange={e=>setD2({...d,waNum:e.target.value})} placeholder="972501234567"/>
        <label style={lbl}>סיסמת ניהול</label><input style={inp} value={d.pass||DEF.pass} onChange={e=>setD2({...d,pass:e.target.value})}/>
        <button onClick={saveAll} className="btn btn-a" style={{marginTop:8,padding:"10px 24px",fontSize:14}}>💾 שמור</button>
      </div>
    </div>}

    {tab==="services"&&<div>
      <h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>💰 שירותים <button onClick={addS} style={{fontSize:12,background:"rgba(200,164,78,.1)",border:"none",color:"#C8A44E",padding:"4px 12px",borderRadius:8,cursor:"pointer",marginRight:8}}>+ הוסף</button></h3>
      {d.services.map((s,i)=><div key={i} style={box}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><strong style={{color:"#C8A44E"}}>שירות {i+1}</strong><button onClick={()=>delS(i)} style={{background:"none",border:"none",color:"#F87171",cursor:"pointer",fontSize:12}}>🗑 מחק</button></div>
        <label style={lbl}>שם</label><input style={inp} value={s.name} onChange={e=>upS(i,"name",e.target.value)}/>
        <label style={lbl}>תיאור</label><textarea style={{...inp,height:60,resize:"vertical"}} value={s.desc} onChange={e=>upS(i,"desc",e.target.value)}/>
        <div style={{display:"flex",gap:8}}><div style={{flex:1}}><label style={lbl}>מחיר (₪)</label><input style={inp} type="number" value={s.price} onChange={e=>upS(i,"price",e.target.value)}/></div><div style={{flex:1}}><label style={lbl}>צבע</label><input style={{...inp,padding:4,height:38}} type="color" value={s.accent||"#C8A44E"} onChange={e=>upS(i,"accent",e.target.value)}/></div></div>
      </div>)}
      <button onClick={saveAll} className="btn btn-a" style={{marginTop:8,padding:"10px 24px",fontSize:14}}>💾 שמור</button>
    </div>}

    {tab==="reviews"&&<div>
      <h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>⭐ ביקורות <button onClick={addR} style={{fontSize:12,background:"rgba(200,164,78,.1)",border:"none",color:"#C8A44E",padding:"4px 12px",borderRadius:8,cursor:"pointer",marginRight:8}}>+ הוסף</button></h3>
      {d.reviews.map((r,i)=><div key={i} style={box}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><strong style={{color:"#C8A44E"}}>ביקורת {i+1}</strong><button onClick={()=>delR(i)} style={{background:"none",border:"none",color:"#F87171",cursor:"pointer",fontSize:12}}>🗑</button></div>
        <label style={lbl}>שם</label><input style={inp} value={r.n} onChange={e=>upR(i,"n",e.target.value)}/>
        <label style={lbl}>ביקורת</label><textarea style={{...inp,height:60,resize:"vertical"}} value={r.t} onChange={e=>upR(i,"t",e.target.value)}/>
        <label style={lbl}>שירות</label><input style={inp} value={r.s} onChange={e=>upR(i,"s",e.target.value)}/>
      </div>)}
      <button onClick={saveAll} className="btn btn-a" style={{marginTop:8,padding:"10px 24px",fontSize:14}}>💾 שמור</button>
    </div>}

    {tab==="ba"&&<div>
      <h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>📸 לפני / אחרי <button onClick={addB} style={{fontSize:12,background:"rgba(200,164,78,.1)",border:"none",color:"#C8A44E",padding:"4px 12px",borderRadius:8,cursor:"pointer",marginRight:8}}>+ הוסף</button></h3>
      {d.beforeAfter.map((b,i)=><div key={i} style={box}>
    ...
    {tab==="areas"&&<div>
      <h3 style={{fontSize:18,color:"#fff",marginBottom:16}}>🗺️ אזורי שירות</h3>
      <div style={box}><label style={lbl}>ערים (מופרדות ב-·)</label><textarea style={{...inp,height:100,resize:"vertical"}} value={d.areas||DEF.areas} onChange={e=>setD2({...d,areas:e.target.value})}/><button onClick={saveAll} className="btn btn-a" style={{marginTop:8,padding:"10px 24px",fontSize:14}}>💾 שמור</button></div>
    </div>}
