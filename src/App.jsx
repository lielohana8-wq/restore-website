...
<section className="sec" id="faq"><div className="mx" style={{maxWidth:680}}><Fade><STit>שאלות נפוצות</STit></Fade>{[
["כמה זמן לוקח ניקוי ספה?","תהליך הניקוי עצמו אורך בין 30 ל-60 דקות, בהתאם לגודל הספה (דו מושבית, תלת, פינתית), סוג הבד ורמת הלכלוך. לאחר הניקוי, זמן הייבוש נע בין 4 ל-6 שעות — תלוי בסוג הבד, רמת הלחות בחדר והאוורור. ספות עור או דמוי עור מתייבשות מהר יותר (2-3 שעות). אנחנו ממליצים לפתוח חלון או להדליק מאוורר כדי לזרז את הייבוש. במקרים דחופים, אנחנו יכולים להביא מאוורר ייבוש תעשייתי שמקצר את הזמן משמעותית."],
["האם החומרים בטוחים לילדים ולחיות מחמד?","בהחלט. אנחנו עובדים אך ורק עם חומרים היפואלרגניים, ללא כלור, ללא אמוניה, ומאושרים לשימוש ביתי על ידי משרד הבריאות. החומרים שלנו בטוחים לתינוקות, ילדים קטנים, נשים בהריון, אנשים עם אסטמה או אלרגיות, וכל חיות המחמד — כלבים, חתולים וכו׳. אין ריחות כימיים חזקים. להפך — אחרי הניקוי יש ריח נעים ורענן. כל החומרים מתפרקים ביולוגית ולא משאירים שאריות רעילות על הבד."],
["אתם מנקים כל סוגי הספות?","כן, אנחנו מתמחים בכל סוגי הבדים והחומרים: ספות בד (כותנה, פוליאסטר, פשתן), ספות עור אמיתי, עור סינטטי, דמוי עור, אימפלה, קטיפה, מיקרופייבר, אלקנטרה, זוהרית, ועוד. לכל סוג בד יש שיטת ניקוי מותאמת — טמפרטורת מים שונה, חומרי ניקוי ייעודיים ולחץ שאיבה מתאים. לפני כל טיפול אנחנו מבצעים אבחון של סוג הבד ובודקים את תגובתו לחומר בנקודה נסתרת. שלחו לנו תמונה של הספה ונוכל לייעץ מיד."],
["כמה עולה ניקוי ספה?","ניקוי ספה מתחיל מ-₪"+(D.services?.[0]?.price||250)+" בלבד, כולל חיטוי אנטיבקטריאלי ובישום. המחיר הסופי נקבע לפי: גודל הספה (דו מושבית / תלת / פינתית / L), סוג הבד, מספר וחומרת הכתמים, והאם נדרש טיפול מיוחד (כמו הסרת שיער חיות, עובש, או ריח שתן). ללקוחות שמזמינים כמה פריטים (ספה + מזרן למשל) יש הנחות חבילה. הדרך הכי מהירה לקבל מחיר — שלחו תמונה בוואטסאפ ותקבלו הצעה מדויקת תוך דקות, ללא התחייבות."],
["מה אם הכתם לא יורד?","אנחנו מתחייבים לתוצאה הטובה ביותר. מניסיון — 95% מהכתמים יורדים לחלוטין כבר בטיפול הראשון, כולל כתמי קפה, שוקולד, יין, שתן, הקאות ושומן. לגבי כתמים עקשנים במיוחד (כמו כתמי דיו, צבע, או חלודה) — אנחנו תמיד מעדכנים את הלקוח מראש לגבי הסיכויים, ולא מבטיחים מה שלא נוכל לספק. במקרים קשים אנחנו מציעים טיפול חוזר או פתרון חלופי. השקיפות שלנו היא הדבר שהלקוחות הכי מעריכים."],
["לאן אתם מגיעים?","פריסה ארצית! אנחנו מכסים את כל אזור גוש דן (תל אביב, רמת גן, גבעתיים, בני ברק, חולון, בת ים, פתח תקווה, ראשון לציון), השרון (הרצליה, רעננה, כפר סבא, נתניה, הוד השרון), השפלה (רחובות, נס ציונה, לוד, רמלה), ירושלים ובית שמש, חיפה והקריות, באר שבע, אשדוד, אשקלון, מודיעין ועוד. הגעה חינם ברוב האזורים. שלחו כתובת בוואטסאפ ונאשר תוך דקות."],
["האם אתם מביאים את כל הציוד?","כן, אנחנו מגיעים מצוידים בכל מה שצריך — מכונת שאיבה תעשייתית בעלת עוצמה גבוהה, מכונת הזרקה-שאיבה מקצועית, חומרי ניקוי וחיטוי ייעודיים לכל סוג בד, מברשות מקצועיות, ציוד חיטוי באדים, וחומרי בישום. אתם לא צריכים להכין כלום מראש — רק לפנות גישה נוחה לספה ולוודא שיש שקע חשמל בקרבת מקום. הציוד שלנו שקט יחסית ולא יפריע לשכנים."],
["אפשר לשבת על הספה אחרי הניקוי?","מומלץ להמתין 4-6 שעות עד שהספה מתייבשת לחלוטין. בזמן הזה כדאי לאוורר את החדר ולהימנע משימוש בספה. אחרי הייבוש המלא — הספה מוכנה לשימוש מיידי. תרגישו הבדל מיידי: הבד רך ונקי, הריח נעים ורענן, והצבע חוזר להיות עשיר ובהיר. הרבה לקוחות אומרים שזה מרגיש כמו ספה חדשה. אנחנו גם ממליצים לא להניח כריות רטובות או שמיכות על הספה עד לייבוש מלא."],
["אתם גם מנקים מזרנים, שטיחים ורכבים?","כן! מלבד ספות אנחנו מציעים מגוון שירותי ניקוי וחיטוי: מזרנים (מ-₪"+(D.services?.[1]?.price||150)+" — כולל הסרת קרדית אבק, כתמים ובישום), שטיחים (ניידים ומקיר לקיר, מ-₪"+(D.services?.[4]?.price||200)+"), כיסאות (אוכל, משרדיים, גיימינג, מ-₪"+(D.services?.[5]?.price||80)+" ליחידה), פנים רכבים (ריפודים, תקרה, דשבורד, תא מטען, מ-₪"+(D.services?.[3]?.price||300)+"), ומזגנים (ניקוי וחיטוי, מ-₪"+(D.services?.[2]?.price||150)+"). ללקוחות שמזמינים כמה שירותים יחד יש הנחות חבילה משמעותיות!"],
["יש אחריות על העבודה?","כן, אנחנו נותנים אחריות מלאה על כל עבודה שאנחנו מבצעים. אם לא שבעי רצון מהתוצאה — פשוט תגידו לנו ונחזור לבצע טיפול חוזר ללא עלות נוספת. אנחנו עובדים בשקיפות מלאה: לפני הטיפול נסביר בדיוק מה צפוי, מה אפשר להשיג ומה לא, ולא נגבה כסף על שירות שלא סופק. יש לנו אלפי לקוחות מרוצים ודירוג 5 כוכבים. שביעות הרצון שלכם היא לא סתם סיסמא — זה הבסיס של העסק שלנו."],
["איך מזמינים?","פשוט מאוד! שלחו לנו הודעה בוואטסאפ עם תמונה של הפריט שצריך ניקוי (ספה, מזרן, שטיח וכו׳). תוך דקות תקבלו הצעת מחיר מדויקת. אם מתאים — נתאם מועד שנוח לכם (עובדים 7 ימים בשבוע, כולל ערבים). מגיעים, מנקים, ונשאיר את הבית שלכם כמו חדש. תשלום במזומן, ביט, פייבוקס או אשראי."]
].map(([q,a],i)=><Fade key={i} d={i*.04}><FaqItem q={q} a={a}/></Fade>)}</div></section>
